...
const Login  = () => {
    ...
    const onSubmit = data => {
        loginUser(data.email, data.password,data.name,history,location);
  ...
  <div className="mr-auto ml-auto w-full">
    <div className="w-full max-w-md mr-auto ml-auto mt-4 mb-1 text-left">
      <h1 className="text-gray-800 block text-3xl font-extrabold font-title">Login</h1>
    </div>
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="w-full max-w-md mr-auto ml-auto mt-4">
      <div className="bg-white shadow-lg rounded-md px-8 py-8 mb-4 ml-auto mr-auto">
        ...
        <div className="mb-4">
          ...
          <input className="shadow-sm appearance-none border border-gray-400 rounded w-full py-4 px-3 text-gray-700 text-sm leading-tight focus:outline-none focus:border-indigo-300" {...register("email")} id="username" type="text" placeholder="Your Email"/>
        </div>
        <div className="mb-6">
  ...
  <p className="font-bold">{authError}</p>
        ...
        <NavLink 
               ...
               to='/Register'
               ><button variant="text"> please Register</button></NavLink>
        </div>
